...
from nlpre import titlecaps, dedash, identify_parenthetical_phrases, separate_reference, unidecoder, pos_tokenizer
...
def on_match(matcher, doc, i, matches):
    ...
    try:
        ...
        print("Conflicting entity: " + entity.text)
...
def process_text(data, tagging_data):
    ...
    for text in data:
        corpus += " " + text
    parsers = [dedash(), titlecaps(), separate_reference(), unidecoder()]#, replace_from_dictionary(prefix="MeSH_")]
    for parser in parsers:
        corpus = parser(corpus)
    HP_patterns = [nlp(text) for text in tagging_data["HPO"]]
    HP_Syn_Patterns = [nlp(text) for text in tagging_data["HPO_Syn"]]
    MeSH_patterns = [nlp(text) for text in tagging_data["MeSH"]]
    ...
    phrase_matcher.add('MeSH', on_match, *MeSH_patterns)
    phrase_matcher.add('HP', on_match, *HP_patterns)
    phrase_matcher.add('HP_Syn', on_match, *HP_Syn_Patterns)
